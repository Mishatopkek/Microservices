...
public class CheckoutOrderCommandHandler : IRequestHandler<CheckoutOrderCommand, int>
{
    ...
    private async Task SendMail(Order order)
    {
        Email email = new Email()
            {To = "mishatopkek@gmial.com", Body = "Order was created.", Subject = "Order was created"};
        ...
        {
            _logger.LogError("Order {OrderId} failed due to an error with the mail service: {EMessage}", order.Id, e.Message);
        }
    }
}